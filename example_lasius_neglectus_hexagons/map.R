setwd("/Users/charlesrocabert/git/MoRIS-development/example_lasius_neglectus")
...
plot_map <- function( d, xintro, yintro )
{
	...
	STEP   = 0.1
	FVEC   = seq(0.0, 1.0-STEP, by=STEP)
	colors = colorRampPalette(c("blue", "red"))(n=length(FVEC))
	...
	dth = d[d$n > 0,]
	points(dth$xcoord, dth$ycoord, pch=".", col="black")
	dth = d[d$y>0,]
	...
	legend("topleft", legend=c("0.0 < f <= 0.1", "0.1 < f <= 0.2", "0.2 < f <= 0.3", "0.3 < f <= 0.4", "0.4 < f <= 0.5", "0.5 < f <= 0.6", "0.6 < f <= 0.7", "0.7 < f <= 0.8", "0.8 < f <= 0.9", "0.9 < f <= 1.0", "Positive observation", "Sampling", "Introduction"), col=c(colors, "green", "black", "black"), pch=c(rep(15, 11), 20, NA), lty=c(rep(NA, 12), 2), cex=0.8)
}

d = read.table("final_state.txt", h=T, sep=" ")
p = read.table("params_out.txt", h=T, sep=" ")
